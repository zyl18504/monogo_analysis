...
class MongoDumpMiddleware(object):
    ...
    def process_response(self, request, response):
        if settings.DEBUG and hasattr(self, 'orig_send_message') and hasattr(self, 'orig_send_message_with_response'):
            ...
            log_file = open(settings.MONGO_ANALYSIS_FILE,'ab')
            ...
            MongoReplicaSetClient._send_message = \
                    self.orig_rs_send_message
            MongoReplicaSetClient._send_message_with_response = \
            now_time_str = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S')
            log_info = transfer_mongo_log(connection.queries)
            log_file.write('%s %s %s' % (request.path,json.dumps(log_info),now_time_str))
            log_file.write('\r\n')
        return response
    ...
    def _instrument(self, original_method):
        def instrumented_method(*args, **kwargs):
            ...
            self._used_msg_ids.append(message.pop('msg_id'))
            start = time()
...
def _mongodb_decode_wire_protocol(message):
    ...
    return { 'op': op, 'collection': collection_name,
             'msg_id': msg_id,'query': msg }


def transfer_mongo_log(mongo_log):
    result_dict = {}
    total_cost = 0
    for info in mongo_log:
        collection_name = info['mongo']['collection']
        op = info['mongo']['op']
        cost = float(info['time'])
        if collection_name not in result_dict:
            result_dict[collection_name] = {'cost':0}
        if op not in result_dict[collection_name]:
            result_dict[collection_name][op] = 0
        result_dict[collection_name][op] += 1
        result_dict[collection_name]['cost'] += float('%.3f' % cost)
        total_cost += cost
    result_dict['total'] = round(total_cost,4)
